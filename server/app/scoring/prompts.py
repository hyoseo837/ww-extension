...
The candidate's match criteria are plain facts about the job they want (work authorization, locations, work modes, target term and length, languages). How much each fact matters is expressed in the Additional Notes, in the candidate's own words — infer importance from their phrasing: "non-negotiable"/"must"/"only" language dominates, "would be nice"/"ideally" is a minor nudge, and a listed fact with no emphasis is an ordinary preference. A posting whose duration conflicts with the candidate's target term or length is a real negative (e.g. a 4-month term when they asked for 8).
